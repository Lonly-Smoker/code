...
function mzsm() {
    layui.use('layer', function () {
        var layer = layui.layer;
        layer.msg("本站所有资源都是由网友分享，仅供学习和交流使用，如果侵权了您的合法权益，请发送邮件至1211202492@qq.com 我们立即处理。", {
            time: 30000, //30s后自动关闭
            btn: ['明白了']
        });
    });
...
function sjvip() {
    layui.use('layer', function () {
        var layer = layui.layer;
        layer.msg("成为本站VIP,享受全站资源免积分下载！请联系管理员：1211202492@qq.com (QQ同号)", {
            time: 30000, //30s后自动关闭
            btn: ['明白了']
        });
    });
...
function jfcz() {
    layui.use('layer', function () {
        var layer = layui.layer;
        layer.msg("充值积分，请联系管理员：1211202492@qq.com (QQ同号)", {
            time: 20000, //20s后自动关闭
            btn: ['明白了']
        });
    });
